...
export async function calculateStats(groupId) {
    ...
    const activeExpenses = groupExpenses
        ...
        if (!paidByActive && participants.length === 0)
            return null;
    ...
    activeExpenses.forEach(exp => {
        if (!membersSet.has(exp.paidBy))
            return; // silinən user-in ödədiyi cari statistikaya girməsin
    ...
    if (Object.keys(totalPaidPerUser).length > 0) {
        mostSpender = Object.entries(totalPaidPerUser).sort((a, b) => b[1] - a[1])[0];
    }
